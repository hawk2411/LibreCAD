...
//    RS_DEBUG->print("RS_ActionDrawLineTangent2::mouseMoveEvent begin");
    ...
    deletePreview();
	_preview->addEntity(new RS_Line(_preview.get(), *lineData));
    drawPreview();
}
...
void RS_ActionDrawLineTangent2::updateMouseButtonHints() {
	switch (getStatus()) {
	case SetCircle1:
		GetDialogFactory()->updateMouseWidget(tr("Select first circle or ellipse"),
											tr("Cancel"));
		break;
	case SetCircle2:
		GetDialogFactory()->updateMouseWidget(tr("Select second circle or ellipse"),
											tr("Back"));
		break;
	default:
