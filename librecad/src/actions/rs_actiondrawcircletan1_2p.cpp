...
void RS_ActionDrawCircleTan1_2P::trigger() {
	...
	RS_Circle* c=new RS_Circle(_container, pPoints->cData);

    _container->addEntity(c);
    ...
    case SetCenter: {
        ...
        if(preparePreview()) {
			...
			for(size_t i=0; i<pPoints->centers.size(); ++i)
				_preview->addEntity(new RS_Point(_preview.get(), RS_PointData(pPoints->centers.at(i))));
            _preview->addEntity(e);
...
void RS_ActionDrawCircleTan1_2P::updateMouseButtonHints() {
	...
	case SetPoint1:
		GetDialogFactory()->updateMouseWidget(tr("Specify the first point on the tangent circle"),
											tr("Back"));
		break;

	case SetPoint2:
		GetDialogFactory()->updateMouseWidget(tr("Specify the second point on the tangent circle"),
											tr("Back"));
		break;
	case SetCenter:
