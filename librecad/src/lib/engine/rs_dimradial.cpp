...
RS_DimRadialData::RS_DimRadialData(const RS_Vector &_definitionPoint,
                                   double _leader) :
        _definitionPoint(_definitionPoint), _leader(_leader) {
}

std::ostream &operator<<(std::ostream &os,
                         const RS_DimRadialData &dd) {
    os << "(" << dd._definitionPoint << "/" << dd._leader << ")";
    return os;
}
...
QString RS_DimRadial::getMeasuredLabel() {
    ...
    double dist = _data.definitionPoint.distanceTo(_edata._definitionPoint) * getGeneralFactor();
...
RS_VectorSolutions RS_DimRadial::getRefPoints() const {
    return RS_VectorSolutions({_edata._definitionPoint,
...
void RS_DimRadial::updateDim(bool autoText) {
    ...
    RS_Vector p1 = _data.definitionPoint;
    RS_Vector p2 = _edata._definitionPoint;
    ...
    RS_MTextData textData = RS_MTextData(RS_Vector(0.0, 0.0),
                            dimtxt, 30.0,
...
void RS_DimRadial::move(const RS_Vector &offset) {
    ...
    _edata._definitionPoint.move(offset);
    update();
}
...
void RS_DimRadial::rotate(const RS_Vector &center, const RS_Vector &angleVector) {
    ...
    _edata._definitionPoint.rotate(center, angleVector);
    update();
}
...
void RS_DimRadial::scale(const RS_Vector &center, const RS_Vector &factor) {
    ...
    _edata._definitionPoint.scale(center, factor);
    _edata._leader *= factor.x;
    update();
}
...
void RS_DimRadial::mirror(const RS_Vector &axisPoint1, const RS_Vector &axisPoint2) {
    ...
    _edata._definitionPoint.mirror(axisPoint1, axisPoint2);
    update();
}
...
void RS_DimRadial::moveRef(const RS_Vector &ref, const RS_Vector &offset) {
    ...
    if (ref.distanceTo(_edata._definitionPoint) < 1.0e-4) {
        double d = _data.definitionPoint.distanceTo(_edata._definitionPoint);
        double a = _data.definitionPoint.angleTo(_edata._definitionPoint + offset);

        RS_Vector v = RS_Vector::polar(d, a);
        _edata._definitionPoint = _data.definitionPoint + v;
        updateDim(true);
