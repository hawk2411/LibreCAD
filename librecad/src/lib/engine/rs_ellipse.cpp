...
//functor to solve for distance, used by snapDistance
    ...
    public:
        EllipseDistanceFunctor(RS_Ellipse const *ellipse, double const &target) :
                distance{target}, e{ellipse}, ra{e->getMajorRadius()}, k2{1. - e->getRatio() * e->getRatio()},
                k2ra{k2 * ra} {
        }
...
RS_Vector RS_Ellipse::getNearestPointOnEntity(const RS_Vector &coord,
    ...
    if (roots.size() == 0) {
        ...
        std::cout << "(a= " << a << " b= " << b << " x= " << x << " y= " << y << " )\n";
        std::cout << "finding minimum for (" << x << "-" << a << "*cos(t))^2+(" << y << "-" << b << "*sin(t))^2\n";
        std::cout << "2::find cosine, variable c, solve(c^4 +(" << ce[0] << ")*c^3+(" << ce[1] << ")*c^2+(" << ce[2]
                  << ")*c+(" << ce[3] << ")=0,c)\n";
...
bool RS_Ellipse::createInscribeQuadrilateral(const std::vector<RS_Line *> &lines) {
    ...
    for (int i = 0; i <= 1; ++i) {
        RS_VectorSolutions const &sol1 = RS_Information::getIntersectionLineLine(quad[i].get(),
                                                                                 quad[(i + 2) % 4].get());
        RS_Vector direction;
        if (sol1.getNumber() == 0) {
    ...
    if (parallel == 1) {
        ...
        if (fabs(centerPoint.distanceTo(l0->getStartpoint()) - centerPoint.distanceTo(l0->getEndpoint())) >
            RS_TOLERANCE)
            return false;
        //symmetric
...
RS2::Ending RS_Ellipse::getTrimPoint(const RS_Vector &trimCoord,
    ...
    if (RS_Math::getAngleDifference(angM, data.angle1, isReversed()) >
        RS_Math::getAngleDifference(data.angle2, angM, isReversed())) {
...
double RS_Ellipse::areaLineIntegral() const {
    ...
    const double fStart = cx * getStartpoint().y + 0.25 * r2 * sin(2. * aE) * cos(a0) * cos(a0) -
                          0.25 * ab * (2. * sin(aE) * sin(aE) * sin(2. * a0) - sin(2. * a0));
    const double fEnd = cx * getEndpoint().y + 0.25 * r2 * sin(2. * aE) * cos(a1) * cos(a1) -
                        0.25 * ab * (2. * sin(aE) * sin(aE) * sin(2. * a1) - sin(2. * a1));
