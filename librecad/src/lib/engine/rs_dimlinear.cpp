...
RS_DimLinearData::RS_DimLinearData() :
        _extensionPoint1(false),
        _extensionPoint2(false),
        _angle(0.0),
        _oblique(0.0) {}
...
RS_DimLinearData::RS_DimLinearData(const RS_Vector &_extensionPoint1,
        ...
        _extensionPoint1(_extensionPoint1), _extensionPoint2(_extensionPoint2), _angle(_angle), _oblique(_oblique) {
}

std::ostream &operator<<(std::ostream &os,
                         const RS_DimLinearData &dd) {
    os << "(" << dd._extensionPoint1 << ","
       << dd._extensionPoint1 << ','
       << dd._angle << ','
       << dd._oblique << ','
       << ")";
...
RS_VectorSolutions RS_DimLinear::getRefPoints() const {
    return RS_VectorSolutions({_edata._extensionPoint1, _edata._extensionPoint2,
                               _data.definitionPoint, _data.middleOfText});
}

void RS_DimLinear::setAngle(double a) {
    _edata._angle = RS_Math::correctAngle(a);
}
...
QString RS_DimLinear::getMeasuredLabel() {
    ...
    RS_Vector dirDim = RS_Vector::polar(100.0, _edata._angle);
    ...
    RS_Vector dimP1 = dimLine.getNearestPointOnEntity(_edata._extensionPoint1, true, nullptr, nullptr);
    RS_Vector dimP2 = dimLine.getNearestPointOnEntity(_edata._extensionPoint2, true, nullptr, nullptr);

    // Definitive dimension line:
    const double dist = dimP1.distanceTo(dimP2) * getGeneralFactor();
...
bool RS_DimLinear::hasEndpointsWithinWindow(const RS_Vector &v1, const RS_Vector &v2) {
    return (_edata._extensionPoint1.isInWindow(v1, v2) ||
            _edata._extensionPoint2.isInWindow(v1, v2));
}
...
void RS_DimLinear::updateDim(bool autoText) {
    ...
    */
    ...
    if ((_edata._extensionPoint1 - dimP1).magnitude() < 1e-6) {
        if ((_edata._extensionPoint2 - dimP2).magnitude() < 1e-6) {
            //boot extension points are in dimension line only rotate 90
            extAngle2 = _edata._angle + (M_PI_2);
        } else {
            //first extension point are in dimension line use second
            extAngle2 = _edata._extensionPoint2.angleTo(dimP2);
        }
        extAngle1 = extAngle2;
    } else {
        //first extension point not are in dimension line use it
        extAngle1 = _edata._extensionPoint1.angleTo(dimP1);
        if ((_edata._extensionPoint2 - dimP2).magnitude() < 1e-6)
            extAngle2 = extAngle1;
        else
            extAngle2 = _edata._extensionPoint2.angleTo(dimP2);
    ...
    if (getFixedLengthOn()) {
        ...
        double extLength = (_edata._extensionPoint1 - dimP1).magnitude();
        if (extLength - dimexo > dimfxl)
            vDimexo1.setPolar(extLength - dimfxl, extAngle1);
        extLength = (_edata._extensionPoint2 - dimP2).magnitude();
        if (extLength - dimexo > dimfxl)
    ...
    auto *line = new RS_Line{this,
                             _edata._extensionPoint1 + vDimexo1, dimP1 + vDimexe1};
    line->setPen(pen);
//    line->setPen(RS_Pen(RS2::FlagInvalid));
    ...
    line = new RS_Line{this,
                       _edata._extensionPoint2 + vDimexo2, dimP2 + vDimexe2};
    line->setPen(pen);
//    line->setPen(RS_Pen(RS2::FlagInvalid));
    line->setLayer(nullptr);
    addEntity(line);
...
void RS_DimLinear::move(const RS_Vector &offset) {
    ...
    _edata._extensionPoint1.move(offset);
    _edata._extensionPoint2.move(offset);
    update();
}
...
void RS_DimLinear::rotate(const RS_Vector &center, const double &angle) {
    ...
    _edata._extensionPoint1.rotate(center, angleVector);
    _edata._extensionPoint2.rotate(center, angleVector);
    _edata._angle = RS_Math::correctAngle(_edata._angle + angle);
    update();
}
...
void RS_DimLinear::rotate(const RS_Vector &center, const RS_Vector &angleVector) {
    ...
    _edata._extensionPoint1.rotate(center, angleVector);
    _edata._extensionPoint2.rotate(center, angleVector);
    _edata._angle = RS_Math::correctAngle(_edata._angle + angleVector.angle());
    update();
}
...
void RS_DimLinear::scale(const RS_Vector &center, const RS_Vector &factor) {
    ...
    _edata._extensionPoint1.scale(center, factor);
    _edata._extensionPoint2.scale(center, factor);
    update();
}
...
void RS_DimLinear::mirror(const RS_Vector &axisPoint1, const RS_Vector &axisPoint2) {
    ...
    _edata._extensionPoint1.mirror(axisPoint1, axisPoint2);
    _edata._extensionPoint2.mirror(axisPoint1, axisPoint2);

    RS_Vector vec;
    vec.setPolar(1.0, _edata._angle);
    vec.mirror(RS_Vector(0.0, 0.0), axisPoint2 - axisPoint1);
    _edata._angle = vec.angle();
...
void RS_DimLinear::stretch(const RS_Vector &firstCorner,
    ...
    if (getMin().isInWindow(firstCorner, secondCorner) &&
        ...
        if (_edata._extensionPoint1.isInWindow(firstCorner,
                                               secondCorner)) {
            _edata._extensionPoint1.move(offset);
        }
        if (_edata._extensionPoint2.isInWindow(firstCorner,
                                               secondCorner)) {
            _edata._extensionPoint2.move(offset);
        }
...
void RS_DimLinear::moveRef(const RS_Vector &ref, const RS_Vector &offset) {
    ...
    if (ref.distanceTo(_data.definitionPoint) < 1.0e-4) {
        ...
        updateDim(false);
    } else if (ref.distanceTo(_edata._extensionPoint1) < 1.0e-4) {
        _edata._extensionPoint1 += offset;
        updateDim(true);
    } else if (ref.distanceTo(_edata._extensionPoint2) < 1.0e-4) {
        _edata._extensionPoint2 += offset;
        updateDim(true);
    }
}
