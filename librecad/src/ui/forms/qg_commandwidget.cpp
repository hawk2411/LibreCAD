...
bool QG_CommandWidget::eventFilter(QObject */*obj*/, QEvent *event) {
    if (event->type() == QEvent::KeyPress) {
        ...
        event->accept();
        this->changeFocus();
        auto *newEvent = new QKeyEvent(*dynamic_cast<QKeyEvent *>(event));
...
void QG_CommandWidget::changeFocus() {
    if (!isActiveWindow())
        activateWindow();
...
void QG_CommandWidget::tabPressed() {
    if (actionHandler) {
        ...
        if (reducedChoice.count() == 1) {
            ...
            QString const &proposal = QG_CommandWidget::getRootCommand(reducedChoice, typed);
...
void QG_CommandWidget::handleKeycode(const QString &code) {
