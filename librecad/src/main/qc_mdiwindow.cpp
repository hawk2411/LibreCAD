...
QC_MDIWindow::QC_MDIWindow(RS_Document *doc, QWidget *parent, Qt::WindowFlags wflags)
    ...
    connect(graphicView, &RS_GraphicView::previous_zoom_state,
            (QC_ApplicationWindow *) parent->window(), &QC_ApplicationWindow::setPreviousZoomEnable);
 ...
 */
bool QC_MDIWindow::fileNewTemplate(const QString &fileName, RS2::FormatType type) {
    RS_DEBUG->print("QC_MDIWindow::fileNewTemplate begin");
    ...
    if (ret) {
        RS_DEBUG->print("QC_MDIWindow::fileNewTemplate: autoZoom");
        graphicView->zoomAuto(false);
    } else
        RS_DEBUG->print("QC_MDIWindow::fileNewTemplate: failed");

    RS_DEBUG->print("QC_MDIWindow::fileNewTemplate end");
    return ret;
}
 ...
 */
bool QC_MDIWindow::fileOpen(const QString &fileName, RS2::FormatType type) {

    RS_DEBUG->print("QC_MDIWindow::fileOpen");
    bool ret = false;
    ...
    if (document && !fileName.isEmpty()) {
        ...
        if (ret) {
            if (fileName.endsWith(".lff") || fileName.endsWith(".cxf")) {
                ...
                RS_DEBUG->print("QC_MDIWindow::fileOpen: autoZoom");
                graphicView->zoomAuto(false);
                RS_DEBUG->print("QC_MDIWindow::fileOpen: autoZoom: OK");
            } else
                graphicView->redraw();
        } else {
            RS_DEBUG->print("QC_MDIWindow::fileOpen: failed");
        }
    } else {
        RS_DEBUG->print("QC_MDIWindow::fileOpen: cancelled");
    }

    RS_DEBUG->print("QC_MDIWindow::fileOpen: OK");
 ...
 *         is invalid.
 */
bool QC_MDIWindow::fileSave(bool &cancelled, bool isAutoSave) {
    RS_DEBUG->print("QC_MDIWindow::fileSave()");
    bool ret = false;
    cancelled = false;
 ...
 *         is invalid.
 */
bool QC_MDIWindow::fileSaveAs(bool &cancelled) {
    RS_DEBUG->print("QC_MDIWindow::fileSaveAs");
    bool ret = false;
    cancelled = false;
